...
- **Artwork assets** (`assets/da/`): user-sourced P5 artwork, used as dimmed backdrops only — never as UI chrome. Current placements: `skyline.jpg` (bottom city silhouette in `.bg-city`, masked fade), `joker-hand.jpg` (level-up overlay backdrop), `stars.jpg` (achievements screen header banner + achievement overlay backdrop), `rue-shibuya.jpg` (profile card watermark). All sit under a dark gradient/veil so content wins; keep any new artwork at ≤0.3 effective opacity. `assets/da/refs/` holds uncut reference sheets (art-book sticker plates, white-background images) — not wired into the UI, they'd need transparent-PNG cutouts first.
