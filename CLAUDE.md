...
- **Character avatar** (`js/ui/characterMii.js`): the stats screen shows a Nintendo-Mii-style chibi (big round head, large glossy eyes, small tee+jeans body) built as an inline SVG string with soft gradients — the user explicitly steered the art through three iterations (angular vector silhouette → rejected "pas très beau"; pixel sprite → rejected; Mii chibi → the direction to keep). Each of the 8 attributes drives one visible transformation, tiers at levels 3/5/8 (`tierFor`): strength = thicker arms then bare flexed biceps + tee prints, health = facial expression (neutral → smile → grin + blush), intelligence = Mii glasses, knowledge = floating book, creativity = hand sparks, wealth = gems, social = buddy silhouettes behind, discipline = earbuds → headset → studio headphones (a nod to the user's Mii reference images). Tee color uses the brand red tokens; keep new accessories in the Mii register (rounded, cute), not the sharp Heist register.
