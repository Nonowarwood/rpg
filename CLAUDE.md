...
- **Character avatar** (stats screen): a real image asset at `assets/avatar/mii.png`, provided by the user — **never code-drawn**. Three code-generated versions (angular SVG silhouette, pixel-art sprite, Mii-style SVG chibi) were each rejected by the user; he wants an actual Nintendo-Mii-quality render, which can't be produced in code. The panel (`.character-panel` in `index.html`) stays `hidden` until the image loads (`statsScreen.js` probes it with an `Image()`), and the desktop 2-column `.stats-cols` grid collapses to one column via `:has(.character-panel[hidden])` in `screens.css`. If the user asks for the avatar to evolve with stats again, do it with additional image files (e.g. variants he exports) or UI badges around the image — not by drawing the character.
