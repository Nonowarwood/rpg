...
- **Character sprite** (`js/ui/characterSprite.js`): the stats screen shows a pixel-art hero (white Joker mask, always on) rendered to a 64×52 `<canvas>` scaled up with `image-rendering: pixelated`. The body is generated from a mirrored 13-col half-grid (`BASE_HALF`, palette chars `.WRDLKAP`) so it stays symmetric; each of the 8 attributes drives one visible scene layer (strength = wider build + pec/ab K-lines via `widenTorso`, intelligence = glasses on the mask, knowledge = floating book, creativity = fist sparks, wealth = gems, social = allied busts behind, discipline = ground/platform/bolts, health = heartbeat then sparkles). Tier thresholds are levels 3/5/8 (`tierFor`). No image assets — the art is char-grid stamps composited in `buildScene()`; a previous vector-SVG version was rejected by the user in favor of sprites, don't go back to it. Iterate on the art with the ASCII preview script pattern (mirror + print) before touching coordinates.
