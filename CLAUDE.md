...
- **Character avatar** (stats screen): real image assets in `assets/avatar/`, exported by the user from Nintendo's Mii Studio — **never code-drawn**. Three code-generated versions (angular SVG silhouette, pixel-art sprite, Mii-style SVG chibi) were each rejected by the user; he wants actual Nintendo-render quality, which can't be produced in code. `AVATAR_VARIANTS` in `statsScreen.js` lists renders with a `when()` condition (first available match wins); each file is probed with an `Image()` and only participates once it exists, so shipping a new variant = dropping the PNG (crop/downscale with `sips -c 1240 660` then `--resampleHeight 640`, like the existing ones). Current matrix: outfit by clock (`isPyjamaTime()`: pyjama before 12h and from 22h, day clothes `mii-jour*.png` in between; a 5-min interval re-renders so the outfit flips at noon) × glasses at Intelligence ≥ 5 (`mii-*lunettes.png`). The panel stays `hidden` with no variant, and the desktop `.stats-cols` grid collapses via `:has(.character-panel[hidden])`. Because Mii Studio can't express most progression, evolution happens *around* the image: `.character-aura` (opacity follows player level) and the `.character-gear` diamond row — 8 stat slots climbing a color rank ladder (bronze lvl 3+ / argent 5+ / or 8+ / diamant-cyan 12+). Keep it that way — accessories drawn on top of the render are forbidden.
- **Artwork assets** (`assets/da/`): user-sourced P5 artwork. Backdrops (dimmed under a dark veil, ≤0.3 effective opacity — content wins): `skyline.jpg` (bottom city silhouette in `.bg-city`), `joker-hand.jpg` (level-up overlay), `stars.jpg` (achievements header banner + achievement overlay), `rue-shibuya.jpg` (profile card watermark). Sticker cutouts (transparent PNGs, chroma-keyed from the user's art-book screenshots via a border flood-fill script — grey plate removed, interior antialiasing preserved): `jour-1..7.png` (MONDAY月…SUNDAY日, shown in the home topbar by weekday, 1 = lundi), `level-up.png` (replaces the level-up overlay's text eyebrow), `meteo-{soleil,nuage,pluie,neige}.png` (live weather chip in the topbar — `js/systems/weatherSystem.js` fetches open-meteo with browser geolocation, 2h localStorage cache, silently hidden on refusal/offline). `assets/da/refs/` holds uncut reference sheets and raw screenshots — not wired into the UI.
...
- **Three type families, one job each** — aligned on the user's P5R font chart. `--font-display` (Bungee, weight 400 only — never request another weight, it'll synthesize a fake bold) anchors anything that should shout: headings, the player name, level numbers, section-head titles. Bungee draws very wide, so display sizes across the CSS are tuned smaller than a condensed face would need — if it looks oversized after a change, shrink the `font-size`, don't add negative letter-spacing. `--font-body` (Arsenal — the chart's free stand-in for P5R's Optima Nova Black; draws ~15% wider than average) is for everything read at length or interacted with. `--font-number` (Libre Franklin 700–900 — the chart's Franklin Gothic) carries data: stat-tile values, XP amounts, counters. The heist lettering additionally swaps ~1 letter in 3 into Archivo Black (`.hl--grotesque`) or a system serif (`.hl--serif`) — that per-letter typeface mixing is P5R's actual ransom-note method per the chart. (History: Anton was tried first and rejected as "trop Fortnite"; Bungee picked from a 4-font comparison; Manrope was the body font before the chart arrived.)
